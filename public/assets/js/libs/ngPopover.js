    ...
    module.provider('ngPopover', function () {
        ...
        $(document).on('click', function (e) {
            if (open) {
                ...
                var isFocusHide = $('#ng-popover').attr('disappear');

                if($(e.target).attr('disappear')){
                    isFocusHide = $(e.target).attr('disappear');
                }

                //点击区域不是popover区域  &&  popover区域的子元素 && !inOverlay &&
                if (!$('#ng-popover').is(e.target) && $('#ng-popover').has(e.target).length == 0 && !inOverlay && isFocusHide == 'false') {
        ...
        this.$get = ['$rootScope', '$templateCache', '$compile', function ($rootScope, $templateCache, $compile) {
            return {
                ...
                open: function (element, scope, options) {
                    ...
                    var isFocusHide = options.disappear ? true : false ;
                    ...
                    var setupClick = function () {
                        ...
                        $('#ng-popover').attr('disappear',isFocusHide);
